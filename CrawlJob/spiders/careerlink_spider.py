...
class CareerlinkSpider(scrapy.Spider):
    ...
    def parse_search_results(self, response):
        job_urls = response.css(
            'a[class*="job-link"][href*="tim-viec-lam"]::attr(href)'
        ).getall()
        ...
        for job_url in job_urls:
            yield response.follow(
                job_url, callback=self.parse_job_detail, meta=response.meta
            )
    ...
    def parse_job_detail(self, response):
        ...
        item["job_description"] = " ".join(
            [d.strip() for d in description if d.strip()]
        )
        ...
        if benefit_items:
            for b_item in benefit_items:
                ...
                clean_text = " ".join(
                    [text.strip() for text in benefit_text if text.strip()]
                )
                if clean_text:
    ...
    def _extract_by_icon(self, response, icon_class):
        text = response.xpath(
            f"//i[contains(@class, '{icon_class}')]/following-sibling::span[1]//text()"
        ).getall()
        if text:
    ...
    def _extract_by_text(self, response, label_text):
        text = response.xpath(
            f'//div[contains(text(), "{label_text}")]/following-sibling::div[1]//text()'
        ).get()
        if text:
            return text.strip()
        return None
