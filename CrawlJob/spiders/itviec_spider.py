...
class ItviecSpider(scrapy.Spider):
    ...
    def _login(self):
        ...
        try:
            email_input = wait.until(
                EC.element_to_be_clickable((By.CSS_SELECTOR, "input[id='user_email']"))
            )
        except TimeoutException:
            self.logger.error(
                "Failed to find the email input field. The page might be blocked or changed."
            )
            self.driver.save_screenshot("itviec_login_error.png")
            with open("itviec_login_error.html", "w", encoding="utf-8") as f:
                f.write(self.driver.page_source)
            self.logger.info(
                "Saved screenshot and page source to 'itviec_login_error.png/html' for debugging."
            )
            return
        ...
        password_input = self.driver.find_element(
            By.CSS_SELECTOR, "input[id='user_password']"
        )
        ...
        try:
            wait.until(
                EC.presence_of_element_located((By.CSS_SELECTOR, "input[id='query']"))
            )
    ...
    def start_requests(self):
        ...
        wait.until(
            EC.presence_of_element_located(
                (By.CSS_SELECTOR, "h1[class*='headline-total-jobs']")
            )
        )
        ...
        try:
            ...
            for selector in next_selectors:
                try:
                    ...
                    self.logger.info(
                        f"Found next page button with selector: {selector}"
                    )
                    break
                except:
                    continue
    ...
    def _extract_job_from_detail(self, job_url):
        ...
        try:
            ...
            try:
                title = preview_job_wrapper.find_element(
                    By.CSS_SELECTOR, "h2[class*='text-it-black text-hover-red']"
                ).text.strip()
            ...
            try:
                company = preview_job_wrapper.find_element(
                    By.CSS_SELECTOR, "a[href*='/companies/'][class*='normal-text']"
                ).text.strip()
            ...
            try:
                salary = preview_job_wrapper.find_element(
                    By.CSS_SELECTOR, "span.ips-2.fw-500"
                ).text.strip()
            ...
            try:
                location = preview_job_wrapper.find_element(
                    By.CSS_SELECTOR, "div.d-inline-block.text-dark-grey"
                ).text.strip()
            ...
            try:
                desc_element = preview_job_wrapper.find_element(
                    By.CSS_SELECTOR, "section[class='job-description']"
                )
            ...
            try:
                req_element = preview_job_wrapper.find_element(
                    By.CSS_SELECTOR, "section[class='job-experiences']"
                )
            ...
            try:
                benefits_element = preview_job_wrapper.find_element(
                    By.CSS_SELECTOR, "section[class='job-why-love-working']"
                )
            ...
            # Other fields
            try:
                industry = preview_job_wrapper.find_element(
                    By.CSS_SELECTOR, "div[class='d-inline-flex text-wrap']"
                ).text.strip()
