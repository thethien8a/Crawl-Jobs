...
class Job123Spider(scrapy.Spider):
    ...
    def parse_job_detail(self, response):
        ...
        # Chi tiết
        item["job_type"] = self._parse_text_in_follow_sibling(
            response, "Hình thức làm việc"
        )

        item["experience_level"] = self._parse_exp_level(response)

        item["education_level"] = self._parse_text_in_follow_sibling(
            response, "Trình độ yêu cầu"
        )

        item["job_industry"] = self._parse_text_in_follow_sibling(
            response, "Ngành nghề"
        )
        ...
        item["job_description"] = self._parse_paragraph_in_follow_sibling(
            response, "Mô tả công việc"
        )
        item["requirements"] = self._parse_paragraph_in_follow_sibling(
            response, "Yêu cầu"
        )
        item["benefits"] = self._parse_paragraph_in_follow_sibling(
            response, "Quyền lợi"
        )
    ...
    def _parse_text_in_follow_sibling(self, response, text_extract):
        text = response.xpath(
            f"//*[contains(text(), '{text_extract}')]/following-sibling::*[1]//text()"
        ).get()
        if text:
            return text.strip()
        return None
    ...
    def _parse_paragraph_in_follow_sibling(self, response, text_extract):
        para = response.xpath(
            f'//h2[contains(normalize-space(.), "{text_extract}")]/following-sibling::*[1]//text()'
        ).getall()
        if para:
            return " ".join(para)
        return None
    ...
    def _parse_exp_level(self, response):
        text = response.xpath(
            f'//div[contains(., "Kinh nghiệm yêu cầu")]/following-sibling::div[1]//text()'
        ).get()
