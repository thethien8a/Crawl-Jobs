...
class CareervietSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        search_url = urljoin(
            base_url, f'viec-lam/{quote(self.keyword.replace(" ", "-"))}-k-vi.html'
        )
    ...
    def parse_job_detail(self, response):
        ...
        # Long texts
        item["job_description"] = self._extract_detail_paragraph(
            response, "Mô tả Công việc"
        )
        item["requirements"] = self._extract_detail_paragraph(
    ...
    def _extract_detail_text(self, response, text_extract):
        value = response.xpath(
            f'//strong[contains(normalize-space(.), "{text_extract}")]/following-sibling::p//text()'
        ).getall()
        if value:
    ...
    def _extract_detail_paragraph(self, response, text_extract):
        texts = response.xpath(
            f'//h2[contains(normalize-space(.), "{text_extract}")]/following-sibling::*//text()'
        ).getall()
        if texts:
