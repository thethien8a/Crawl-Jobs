...
class JobsgoSpider(scrapy.Spider):
    ...
    def parse_search_results(self, response):
        ...
        job_listings = response.css('.job-listing-item, .job-card, [class*="job"]')
        
        if not job_listings:
            # Try alternative selectors
            job_listings = response.css('[class*="job"], [class*="listing"]')
        ...
        for job in job_listings:
            ...
            if job_url:
                if not job_url.startswith('http'):
                    job_url = f"https://jobsgo.vn{job_url}"
        ...
        # Handle pagination
        next_page = response.css('a[rel="next"], .pagination .next a::attr(href)').get()
        if next_page:
            if not next_page.startswith('http'):
                next_page = f"https://jobsgo.vn{next_page}"
            yield scrapy.Request(
                url=next_page,
                callback=self.parse_search_results,
                meta=response.meta
